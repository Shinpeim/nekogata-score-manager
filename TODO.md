...
- [ ] PWAで動いているときには画面をスリープさせたくない
- [ ] いま、キーを選ぶ際"#"も"♭"もあるが、キーとして適切なものに限定したい。つまり…
  - C(Am), D♭(B♭m), D(Bm), E♭(Cm), E(C#m), F(Dm), G♭(E♭m), G(Em), A♭(Fm), A(F#m), B♭(Gm), B(G#m)にしたい
- [ ] 移調機能を作りたい
  - 楽譜自体のキーを変えるとき、「コードも一緒に移調しますか？」と尋ね、ユーザーが「はい」と言ったらコードも一緒に移調する。
