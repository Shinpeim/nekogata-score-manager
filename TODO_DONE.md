...
- [x] **ディグリーネーム（度数表記）表示機能** (実施工数: 2.5時間) ✅ 2025-06-26完了
  ...
  - [x] ディグリー変換ユーティリティ実装（`utils/degreeNames.ts`）
  - [x] 包括的テストスイート実装（19テストケース）
  - [x] ChordGridRendererでディグリー表示（コード名の上に薄い色で表示）
  - [x] ChordChartViewerにトグルスイッチ追加（デフォルトOFF）
  - [x] データマイグレーションv4.0.0実装（古いデータのroot/base再計算）
  - **効果**: 音楽理論学習支援、コード進行の理解向上、データ整合性改善

### 2025-06-26: Dropbox同期の401エラー時リトライ処理実装
- [x] **同期時のリフレッシュトークンによるアクセストークン更新処理の修正** (実施工数: 1時間) ✅ 2025-06-26完了
  - **背景**: 久しぶりに同期すると同期エラーが発生し、再認証で解決する状況
  - **原因**: アクセストークン期限切れ時のリフレッシュ処理が実装されていなかった
  - [x] pull処理のエラーハンドリング改善（エラーを握りつぶさない）
  - [x] 401エラー検出時の自動リトライ処理を全APIメソッドに実装
  - [x] リフレッシュトークンによるアクセストークン自動更新
  - [x] リトライは1回のみ（無限ループ防止）
  - **効果**: 同期機能の安定性向上、再認証頻度の大幅削減

### 2025-06-22: ファイル操作UIの一貫性改善
- [x] **楽譜表示画面から削除・複製ボタンを削除** (実施工数: 30分) ✅ 2025-06-22完了
  - [x] ChordChartActionsコンポーネントから削除・複製ボタンを削除（編集ボタンのみ残す）
  - [x] Score Explorerのアクションドロップダウンに複製機能を追加
  - [x] MainLayoutにhandleDuplicateSelected関数を実装
  - **効果**: ファイル操作をScore Explorerに統一し、UIの一貫性向上

### 2025-06-21: Dropboxリフレッシュトークン対応実装
- [x] **Dropboxアクセストークンの自動リフレッシュ機能実装** (実施工数: 2時間) ✅ 2025-06-21完了
  - **背景**: 現在の実装では4時間ごとに再認証が必要でUX改善の余地あり
  - **現状**: ✅ Authorization Code Flow + PKCE実装済み、✅ 短期トークン対応済み
  - [x] リフレッシュトークンの取得・永続化機能実装（`token_access_type: 'offline'`パラメータ追加）
  - [x] `refreshAccessToken()`メソッド実装（自動リフレッシュロジック）
  - [x] 長期間利用可能な認証状態の実現（4時間制限回避）
  - [x] 利用規約にDropbox認証リスクの明記とセキュリティ情報追加
  - **効果**: 再認証頻度大幅削減、ユーザビリティ向上

### 2025-06-21: 同期機能UX改善（定期同期削除・ドロップダウン化・エラー表示統合）
- [x] **定期同期機能の削除** (実施工数: 1時間)
  - [x] 演奏中の意図しない同期を防止（タイマーによる定期同期を削除）
  - [x] チャート変更時の自動同期は維持（ユーザー操作に連動した同期）
  - [x] useChartSync.tsの109-146行目削除とテスト修正
  - [x] SyncConfig型からsyncIntervalプロパティ削除
  - [x] 設定画面から同期間隔UI削除
  - **効果**: パフォーマンス時の安全性向上、UX改善

- [x] **同期ボタンのドロップダウン化** (実施工数: 2時間)
  - [x] SyncDropdownコンポーネント新規作成
  - [x] 「今すぐ同期」と「詳細設定」の分離でワンクリック同期実現
  - [x] 未認証時の適切なグレーアウト表示
  - [x] 包括的テストスイート追加（14テストケース）
  - **効果**: 同期操作の心理的ハードル削減、アクセシビリティ向上

- [x] **同期エラー表示のSyncStatusIndicator統合** (実施工数: 1.5時間)  
  - [x] モバイルヘッダーから冗長なエラーメッセージ表示を削除
  - [x] SyncStatusIndicatorにクリック可能なエラー詳細ポップアップ追加
  - [x] 外クリックで自動的に閉じる機能実装
  - [x] レスポンシブ対応とモバイル最適化
  - [x] 包括的テストスイート追加（8テストケース）
  - **効果**: モバイルビューでのスペース効率向上、エラー確認のユーザビリティ改善

### 2025-06-18: サクッと片付けタスク3件完了 + E2Eテスト改善
- [x] **JSONエクスポート機能改善** (実施工数: 5分)
  - [x] JSONエクスポート時のインデント除去（ファイルサイズ最適化）
  - **効果**: 1行変更でファイルサイズ削減、ユーザー体験向上

- [x] **モバイルズームイン防止** (実施工数: 5分)
  - [x] metaタグのviewport設定で`user-scalable=no`追加
  - **効果**: モバイル使用時の操作性大幅改善

- [x] **最後に開いていた楽譜の記憶機能** (実施工数: 1時間)
  - [x] LocalStorageに`lastOpenedChartId`を保存する機能追加
  - [x] アプリ起動時に最後の楽譜を自動で開く機能実装
  - **効果**: リロード後の操作ストレス解消、ユーザー体験大幅改善

- [x] **E2Eテスト環境改善**
  - [x] e2e/downloadsディレクトリの適切な管理（.gitkeep + .gitignore設定）
  - [x] 一時ファイル競合回避（タイムスタンプ付きファイル名）
  - [x] ファイルクリーンアップの安定化
  - **効果**: テスト実行の安定性向上、開発環境の改善

### 2025-06-18: E2Eテスト基盤・コア機能完全実装
- [x] **E2Eテスト緊急強化 Phase 1完了** - 音楽アプリのコア機能100%カバー達成
  - [x] **基盤構築**: Playwright環境、Page Object Model、data-testid付与完了
  - [x] **移調機能の完全テスト** - チャート作成→コード入力→移調実行→正確性検証
    - [x] 複数キー移調テスト（C→G→D→A→E）の全コード正確性検証
    - [x] 移調後の表示・保存・復元の完全フロー検証
  - [x] **実際のコード編集テスト** - セクション・コード操作の完全フロー
    - [x] セクション追加（イントロ、Aメロ、Bメロ、サビ）と順序確認
    - [x] コード入力・編集（C, Am, F, G, Dm等）と入力結果検証
    - [x] セクション・コード削除機能、メモ機能の完全テスト
  - [x] **ドラッグ&ドロップ操作テスト** - 順序変更の完全検証
    - [x] コード順序変更（C-Am-F-G → Am-C-G-F）と順序確認
    - [x] セクション順序変更（イントロ-Aメロ-Bメロ → Aメロ-イントロ-Bメロ）
    - [x] 複雑なD&D操作の組み合わせテスト
  - [x] **インポート・エクスポート機能テスト** - データ永続化の完全検証
    - [x] エクスポートダイアログの完全UIフロー（チャート選択→ダイアログ表示→ファイル名確認）
    - [x] エラーハンドリング（不正JSON、無効フォーマット、空ファイル）の包括的テスト
    - [x] CSS問題解決（チェックボックスvisibility問題の根本解決）
  - [x] **テスト品質向上**: データクリーンアップ実装（各テスト前後でLocalStorage初期化）
  - [x] **手動テスト手順書作成** - [MANUAL_TESTING.md](./MANUAL_TESTING.md)
    - [x] Google Drive同期テスト手順（認証・削除同期・コンフリクト処理）
    - [x] PWA・オフライン機能テスト手順
    - [x] LocalForage永続化テスト手順
    - [x] リリース前チェックリスト

### 2025-06-18: E2Eテスト実装（インポート・エクスポート機能）
- [x] **インポート・エクスポート機能のE2Eテスト実装** - PR #130
  - [x] チェックボックスvisibility問題の根本解決（CSS修正: flex items-start → items-center）
  - [x] インデックスベースtest-id実装で安定性向上（UUID → インデックス）
  - [x] エクスポートダイアログの完全E2Eテスト（チャート選択→ダイアログ表示→ファイル名確認）
  - [x] インポート機能のエラーハンドリングE2Eテスト（不正JSON、無効フォーマット、空ファイル）
  - [x] JavaScript経由クリックによるbounding box問題の回避策実装
  - [x] デバッグファイルの整理とクリーンアップ
  - [x] 品質確認三点セット（ユニット565件、ESLint、ビルド）＋E2E 178/180件成功
  - [x] テストカバレッジ分析と妥協判断の適切性評価（7.5/10点の合理的判断）

### 2025-06-18: E2Eテスト実装（コード編集機能）
- [x] **実際のコード編集テスト** - セクション追加・コード入力の完全フロー
  - [x] セクション追加と命名の基本操作（イントロ、Aメロ、Bメロ、サビ）
  - [x] コード入力と編集の基本操作（C, Am, F, G等の入力・確認）
  - [x] 複数セクションでのコード進行作成（4セクション完全フロー）
  - [x] セクション削除機能（中間セクション削除の動作確認）
  - [x] コード削除機能（中間コード削除の動作確認）
  - [x] メモ機能の編集（ノート編集・保存・表示確認）
  - [x] テストデータクリーンアップ実装（各テスト前LocalStorage初期化）
